...
export class PowerPointService {
  ...
  static async addTextbox(text: string, slideIndex?: number): Promise<void> {
    if (typeof PowerPoint !== 'undefined' && Office.context.requirements.isSetSupported('PowerPointApi', '1.4')) {
      return PowerPoint.run(async (context) => {
        ...
        const slide = slideIndex !== undefined
          ? slides.getItemAt(slideIndex)
          : slides.getItemAt(0);
        const textBox = slide.shapes.addTextBox(text);
        // Set a reasonable default position and size so the text is visible
        textBox.left = 50;
        textBox.top = 100;
        textBox.width = 600;
        textBox.height = 100;
  ...
  static async getContextForAI(): Promise<string> {
    try {
      if (typeof PowerPoint === 'undefined' || !Office.context.requirements.isSetSupported('PowerPointApi', '1.4')) {
        // Fallback: use the old getSelectedDataAsync approach
        const slide = await this.getCurrentSlide();
        return `PowerPoint active (limited mode). Selected Text: ${slide.currentSlideText || '(none)'}`;
      }

      // Full presentation context — read ALL slides
      return PowerPoint.run(async (context) => {
        const slides = context.presentation.slides;
        slides.load('items');
        await context.sync();

        let res = `Presentation: ${slides.items.length} slide(s)\n\n`;
        for (let i = 0; i < slides.items.length; i++) {
          ...
          for (const shape of shapes.items) {
            try {
              ...
              // Some shapes don't have text frames
            }
          }
          res += `Slide ${i + 1}: ${slideText.trim() || '(no text)'}\n`;
          if (i < slides.items.length - 1) res += '---\n';
        }
        return res;
      });
    } catch (e) {
